...
async function ancetAnswerLike(interaction) {
    try {
        ...
        if (UserDB && existingUserUserDB ) {
            ...
            existingUserUserDB.couple += UserDB._id;
            await existingUserUserDB.save()

            let text = existingUserUserDB.couple.length < 2 ? `У тебя есть ${existingUserUserDB.liked.length} взаимная симпатия, показать её?` : `У тебя есть ${existingUserUserDB.liked.length} взаимные симпатии, показать их?`
...
async function ancetAnswerYes(interaction) {
    try {
        ...
        if (userDB.couple && userDB.couple.length > 0) {
            userDB.couple.forEach((coupleUser) => {
                console.log(coupleUser);
            });

            return interaction.reply(`Пары пользователя: ${userDB.couple.map(coupleUser => coupleUser.name).join(', ')}`);
        } else {
